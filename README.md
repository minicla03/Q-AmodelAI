...
2. **Flashcard Tool** ⚠️ *In sviluppo*: Genera carte di studio da documenti
3. **Quiz Tool** ⚠️ *In sviluppo*: Crea quiz interattivi basati sul contenuto
...
### Scenario 2: Generazione Flashcard ⚠️ *In sviluppo*
...
### Scenario 3: Creazione Quiz ⚠️ *In sviluppo*
...
User Query → Routing Agent → Tool Selection
    ...
    Document Retrieval (ChromaDB) → Reranking (optional)
                ↓
    Context + Prompt → LLM (Ollama - llama3:latest)
                ↓
            Response + Sources
```
...
🚧 **In sviluppo**
- Generazione flashcard interattive
- Creazione quiz personalizzati
- Miglioramento UI/UX
- Export risposte in vari formati
- Supporto per più formati documenti (DOCX, TXT, etc.)
...
- [Ollama](https://ollama.com/) per l'inference LLM locale
- [Streamlit](https://streamlit.io/) per l'interfaccia utente
- [ChromaDB](https://www.trychroma.com/) per il vector store
- [Hugging Face](https://huggingface.co/) per i modelli di embedding
